...
function createLocalFieldResolver({
  ...
  return async (parent, args, context, info) => {
    ...
    return combine([{ [field.name]: result }, parent])[field.name];
  };
}
...
function combine(results: any[]): any {
  if (results.every(result => isArray(result))) {
    if (results.every(result => result.every((item: any) => item.id))) {
      return Object.values(combine(results.map(result => keyBy(result, 'id'))));
    } else {
    ...
    return merge(
      {},
      ...concat([], ...results.map(result => Object.keys(result))).map(key => ({
        [key]: combine(
          results
    ...
    if (candidates.every(({ type }) => isTypeToInclude(type))) {
      if (candidates.every(({ type }) => isObjectType(type))) {
        ...
        if (candidates.some(({ type }) => isObjectType(type))) {
          throw new Error(
            `Can't merge non-Object type ${
              candidates[0].type.name
            } with Object type of same name`,
          );
        } else {
